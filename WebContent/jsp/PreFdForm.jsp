...
<form method="post" action="<%=request.getContextPath()%>/CustomerRequestServlet">
		...
		<legend>FD details</legend>
			<table class="first">
				<tr>
					<td>FD id</td>
					<td><input type="numeric" name="fdid"></td>
				</tr>
			</table>
		</fieldset>
		<input type="hidden" name="action" value="readPreFD">
	...
	<%
		...
		%>
		<h3><span style="color: red"> <%=request.getAttribute("error5")%></span></h3>
		<%
			}
		%>
	<center><h3><%if(request.getAttribute("requestId")==null){%><%="" %>
	<%}else{ %><%=request.getAttribute("requestId") %><%} %></h3></center>
